...
BEGIN
    ...
    SELECT
        ...
        a.author_name
    FROM dbo.book AS b
        INNER JOIN dbo.book_language AS bl
            ON b.language_id = bl.language_id
        INNER JOIN dbo.publisher AS p
            ON b.publisher_id = p.publisher_id
        INNER JOIN dbo.book_author AS ba
            ON b.book_id = ba.book_id
        INNER JOIN dbo.author AS a
            ON ba.author_id = a.author_id
    WHERE
        (b.rowversion > CONVERT(ROWVERSION, @startRow)
        ...
        OR
        (ba.rowversion > CONVERT(ROWVERSION, @startRow)
